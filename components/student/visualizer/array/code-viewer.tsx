...
export function CodeViewer() {
  const { algorithm, frames, currentFrame } = useVisualizerStore()
  const frame = frames[currentFrame]

  const data = ALGORITHM_DATA[algorithm as keyof typeof ALGORITHM_DATA]
  const codeData = data?.code as any
  const codeLines = (codeData?.pseudo || '// Select an algorithm').split('\n')
              ...
              <div
                ...
                className={cn(
                  ...
                  idx + 1 === activeLineIndex
                    ? 'bg-orange-500/10 text-orange-600 dark:text-orange-300 shadow-[inset_2px_0_0_0_#f97316]'
