...
export function TreeComplexityCard() {
  const { algorithm } = useTreeStore()
  const data = ALGORITHM_DATA[algorithm as keyof typeof ALGORITHM_DATA]

  if (!data)
    return (
      <div className="p-6 bg-white/60 dark:bg-black/40 rounded-3xl border border-white/40 dark:border-white/10 backdrop-blur-xl text-center">
        <span className="text-xs text-zinc-400">
          Complexity data unavailable for {algorithm}
        </span>
      </div>
    )
