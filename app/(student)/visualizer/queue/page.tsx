...
import { QueueCanvas } from '@/components/student/visualizer/queue/queue-canvas'
import { QueueControlDeck } from '@/components/student/visualizer/queue/queue-control-deck'
import { QueueCodeViewer } from '@/components/student/visualizer/queue/queue-code-viewer'
import { QueueExplanationPanel } from '@/components/student/visualizer/queue/queue-explanation-panel'
import { QueueComplexityCard } from '@/components/student/visualizer/queue/queue-complexity-card'
import { QueueInputControls } from '@/components/student/visualizer/queue/queue-input-controls'
...
export default function QueueVisualizerPage() {
  const { reset, setAlgorithm, generateInput, algorithm } = useQueueStore()

  useEffect(() => {
    setAlgorithm('SLIDING_WINDOW_MAX')
    generateInput()
    reset()
  }, [setAlgorithm, generateInput, reset])

  const { isPlaying, playbackSpeed, nextStep } = useQueueStore()
  ...
  return (
    <div className="relative min-h-screen w-full bg-background text-foreground selection:bg-orange-500/20 flex flex-col overflow-x-hidden">
      ...
      <header className="flex-none w-full z-50 px-4 py-3 md:px-6 md:py-4 flex flex-col md:flex-row gap-4 items-center justify-between data-[state=scrolled]:bg-background/80 backdrop-blur-md transition-[background-color]">
          ...
          <div className="flex items-center gap-2 md:gap-4">
            ...
            <div className="flex flex-col">
              <h1 className="text-sm md:text-xl font-black tracking-tighter uppercase italic flex items-center gap-2">
                ...
                <span className="hidden sm:inline">Queue Visualizer</span>
                <span className="sm:hidden">Queue Vis</span>
              </h1>
        ...
        <div className="flex w-full md:w-auto items-center justify-between gap-4">
          ...
          >
            ...
            <SelectContent>
              <SelectItem value="SLIDING_WINDOW_MAX">
                Sliding Window Max
              </SelectItem>
              <SelectItem value="CIRCULAR_QUEUE">Circular Queue</SelectItem>
              <SelectItem value="TASK_SCHEDULING">Task Scheduling</SelectItem>
              <SelectItem value="PRIORITY_QUEUE_SIM">Priority Queue</SelectItem>
            </SelectContent>
          </Select>
