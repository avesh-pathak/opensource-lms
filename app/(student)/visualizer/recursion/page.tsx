...
import { RecursionCanvas } from '@/components/student/visualizer/recursion/recursion-canvas'
import { RecursionControlDeck } from '@/components/student/visualizer/recursion/recursion-control-deck'
import { RecursionCodeViewer } from '@/components/student/visualizer/recursion/recursion-code-viewer'
import { RecursionExplanationPanel } from '@/components/student/visualizer/recursion/recursion-explanation-panel'
import { RecursionComplexityCard } from '@/components/student/visualizer/recursion/recursion-complexity-card'
import { RecursionInputControls } from '@/components/student/visualizer/recursion/recursion-input-controls'
...
export default function RecursionVisualizerPage() {
  ...
  return (
    <div className="relative min-h-screen w-full bg-background text-foreground selection:bg-rose-500/20 flex flex-col overflow-x-hidden">
      {/* Background Atmosphere */}
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[1000px] h-[600px] bg-rose-500/10 blur-[120px] rounded-full -z-10 opacity-30 pointer-events-none" />
      ...
      <header className="flex-none w-full z-50 px-4 py-3 md:px-6 md:py-4 flex flex-col md:flex-row gap-4 items-center justify-between data-[state=scrolled]:bg-background/80 backdrop-blur-md transition-[background-color]">
          ...
          <div className="flex items-center gap-2 md:gap-4">
            ...
            <div className="flex flex-col">
              <h1 className="text-sm md:text-xl font-black tracking-tighter uppercase italic flex items-center gap-2">
                <div className="w-5 h-5 md:w-6 md:h-6 rounded-md bg-rose-500 flex items-center justify-center">
                  <Infinity className="h-3 w-3 text-white" />
                </div>
                <span className="hidden sm:inline">Recursion Visualizer</span>
                <span className="sm:hidden">Recursion Vis</span>
              </h1>
              <span className="text-[9px] md:text-[10px] font-bold tracking-widest text-muted-foreground uppercase">
                Temporal Flow & Call Stacks
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="flex w-full md:w-auto items-center justify-between gap-4">
          <Select value={algorithm} onValueChange={handleAlgorithmChange}>
            ...
            <SelectContent>
              <SelectItem value="REC_FIBONACCI">Fibonacci Tree</SelectItem>
              <SelectItem value="REC_TOWER_OF_HANOI">Tower of Hanoi</SelectItem>
              <SelectItem value="REC_FACTORIAL">Factorial Stack</SelectItem>
              <SelectItem value="REC_SUBSETS">
                Subsets (Backtracking)
              </SelectItem>
            </SelectContent>
          </Select>

          <RecursionInputControls />
        </div>
      </header>
        ...
        <div className="relative flex flex-col flex-1 lg:w-[70%] min-h-[500px] lg:h-full overflow-hidden">
          <div className="flex-1 flex flex-col items-center justify-center relative min-h-[300px] md:min-h-[400px]">
            <div className="w-full h-full flex flex-col items-center z-10">
              ...
              <div className="mt-2 md:mt-8 gap-4 w-full max-w-2xl px-4 flex justify-center pb-8 border-t border-dashed border-zinc-200 dark:border-white/10 pt-8">
                <div className="w-full">
                  <RecursionExplanationPanel />
                </div>
              </div>
            </div>
          </div>
