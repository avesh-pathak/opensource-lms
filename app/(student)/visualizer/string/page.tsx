...
import { StringInputControls } from '@/components/student/visualizer/string/string-input-controls'
import Link from 'next/link'
import { ArrowLeft, TextCursorInput } from 'lucide-react'
...
export default function StringVisualizerPage() {
  ...
  useEffect(() => {
    setAlgorithm('PALINDROME_CHECK')
    setTimeout(() => generateArray(), 100) // Small delay to ensure logic picks up
    reset()
  ...
  const handleAlgorithmChange = (algo: string) => {
    // @ts-expect-error - AlgorithmType union is extensive, using string for flexibility
    setAlgorithm(algo)
    generateArray()
    reset()
  }
  ...
  return (
    <div className="relative min-h-screen w-full bg-background text-foreground selection:bg-pink-500/20 flex flex-col overflow-x-hidden">
      {/* Background Atmosphere - Pink/Purple for Strings */}
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[1000px] h-[600px] bg-pink-500/10 blur-[120px] rounded-full -z-10 opacity-30 pointer-events-none" />
      ...
      <header className="flex-none w-full z-50 px-4 py-3 md:px-6 md:py-4 flex flex-col md:flex-row gap-4 items-center justify-between data-[state=scrolled]:bg-background/80 backdrop-blur-md transition-[background-color]">
          ...
          <div className="flex items-center gap-2 md:gap-4">
            ...
            <div className="flex flex-col">
              <h1 className="text-sm md:text-xl font-black tracking-tighter uppercase italic flex items-center gap-2">
                <div className="w-5 h-5 md:w-6 md:h-6 rounded-md bg-pink-500 flex items-center justify-center">
                  <TextCursorInput className="h-3 w-3 text-white" />
                </div>
                <span className="hidden sm:inline">String Visualizer</span>
                <span className="sm:hidden">String Vis</span>
              </h1>
        ...
        <div className="flex w-full md:w-auto items-center justify-between gap-4">
          ...
          >
            ...
            <SelectContent>
              <SelectItem value="PALINDROME_CHECK">Palindrome Check</SelectItem>
              <SelectItem value="REVERSE_STRING">Reverse String</SelectItem>
              <SelectItem value="LONGEST_SUBSTRING">
                Longest Substr w/o Repeats
              </SelectItem>
              <SelectItem value="LONGEST_PALINDROMIC_SUBSTRING">
                Longest Palindromic Substr
              </SelectItem>
              <SelectItem value="COUNT_PALINDROMIC_SUBSTRINGS">
                Count Palindromic Substrs
              </SelectItem>
              <SelectItem value="LONGEST_COMMON_SUBSEQUENCE">
        ...
        <div className="flex-none w-full lg:w-[30%] lg:border-l lg:border-dashed lg:border-zinc-200 dark:lg:border-white/10 lg:bg-white/30 dark:lg:bg-transparent lg:backdrop-blur-xl overflow-y-auto scrollbar-hide flex flex-col gap-4 p-4 lg:p-6 lg:h-full z-30 bg-white/50 dark:bg-black/50 border-t border-dashed lg:border-t-0">
          <div className="flex flex-col gap-4 h-full">
            ...
            <div className="flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-zinc-500 mb-2">
              <div className="h-1 w-1 bg-pink-500 rounded-full" />
              Algorithm Details
            </div>
