...
import type { TwoPointersAlgorithmType } from '@/lib/store/two-pointers-visualizer-store'
import { TwoPointersCanvas } from '@/components/student/visualizer/two-pointers/two-pointers-canvas'
import { TwoPointersControlDeck } from '@/components/student/visualizer/two-pointers/two-pointers-control-deck'
import { TwoPointersCodeViewer } from '@/components/student/visualizer/two-pointers/two-pointers-code-viewer'
import { TwoPointersExplanationPanel } from '@/components/student/visualizer/two-pointers/two-pointers-explanation-panel'
import { TwoPointersComplexityCard } from '@/components/student/visualizer/two-pointers/two-pointers-complexity-card'
import { TwoPointersInputControls } from '@/components/student/visualizer/two-pointers/two-pointers-input-controls'
...
export default function TwoPointersVisualizerPage() {
  ...
  useEffect(() => {
    setAlgorithm('TP_TWO_SUM_SORTED')
    generateInput()
    reset()
  ...
  const handleAlgorithmChange = (val: string) => {
    setAlgorithm(val as TwoPointersAlgorithmType)
  }

  return (
    <div className="relative min-h-screen w-full flex flex-col bg-background text-foreground selection:bg-zinc-500/20 overflow-x-hidden">
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[1000px] h-[600px] bg-zinc-500/10 blur-[120px] rounded-full -z-10 opacity-30 pointer-events-none" />
      ...
      <header className="flex-none w-full z-50 px-4 py-3 md:px-6 md:py-4 flex flex-col md:flex-row gap-4 items-center justify-between">
          ...
          <div className="flex items-center gap-2 md:gap-4">
            ...
            <div className="flex flex-col">
              <h1 className="text-sm md:text-xl font-black tracking-tighter uppercase italic flex items-center gap-2">
                <div className="w-5 h-5 md:w-6 md:h-6 rounded-md bg-zinc-600 flex items-center justify-center">
                  <MoveHorizontal className="h-3 w-3 text-white" />
                </div>
                <span className="hidden sm:inline">Two Pointers</span>
                <span className="sm:hidden">TP Vis</span>
              </h1>
        ...
        <div className="flex w-full md:w-auto items-center justify-between gap-4">
          <Select value={algorithm} onValueChange={handleAlgorithmChange}>
            ...
            <SelectContent>
              <SelectItem value="TP_TWO_SUM_SORTED">
                Two Sum (Sorted)
              </SelectItem>
              <SelectItem value="TP_PALINDROME">Palindrome Check</SelectItem>
              <SelectItem value="TP_SORT_COLORS">
                Sort Colors (Dutch Flag)
              </SelectItem>
              <SelectItem value="TP_CONTAINER">
                Container With Most Water
              </SelectItem>
            </SelectContent>
          </Select>

          <TwoPointersInputControls />
        </div>
      </header>
        ...
        <div className="flex-none w-full lg:w-[30%] lg:border-l lg:border-dashed lg:border-zinc-200 dark:lg:border-white/10 lg:bg-white/30 dark:lg:bg-transparent lg:backdrop-blur-xl overflow-y-auto scrollbar-hide flex flex-col gap-4 p-4 lg:p-6 lg:h-full z-30 bg-white/50 dark:bg-black/50 border-t border-dashed lg:border-t-0">
          <div className="flex flex-col gap-4 h-full">
            ...
            <div className="flex flex-col gap-4">
              <TwoPointersComplexityCard />
            </div>

            <div className="flex-1 min-h-[300px] flex flex-col">
              <TwoPointersCodeViewer />
            </div>
          </div>
        </div>
      </div>
